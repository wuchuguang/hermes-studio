...
export default {
  ...
  dshPlugins: {
    ...
    "listTab": "插件列表",
    "webPackages": "已安装的 Web 插件",
    "presetEntries": "预设内的插件",
    "packageVersion": "版本",
    "registeredBundle": "已登记到 Web profile",
    "noWebPackages": "尚未安装额外的 Web 插件",
    "webInstallHint": "使用 包名{'@'}精确版本 或 github:作者/仓库#commit 安装或更新。后续 DSH 对话轮次加载新版本。",
    "nativeOperationFailed": "Web 插件操作失败，请刷新列表检查实际安装状态。",
    "settingsUnavailable": "无法读取插件配置，请检查 DSH 安装及 Web profile 后刷新。",
    "nativeHint": "以下是预设内的插件条目。切换预设只改变预览，不修改默认预设；启停标签表示配置状态。",
    ...
    "disabled": "配置已禁用",
    "remove": "卸载",
    "removeConfirm": "从后续启动中卸载 {name}？",
    "conflict": "配置已被其他操作修改。草稿已保留，请对比最新版本后重新加载。",
    "missingDependency": "请先安装 DSH 和 pnpm。",
  },
  ...
  ekkoConfig: {
    ...
    "settingsModules": "模块",
    "settingsAdvanced": "高级",
    "noDefaultProvider": "不设默认 Provider",
    "authorized": "已授权",
    "maxSteps": "最大工具轮次",
    "maxModelRetries": "模型重试次数",
    "maxToolFailures": "同一工具失败纠错阈值",
    "backgroundDelegation": "后台委派",
    "subtaskMaxSteps": "子任务工具轮次",
    ...
    "authorizationLeewayMs": "授权提前刷新时间（毫秒）",
    "toolsEnabled": "启用工具",
    "toolTimeoutMs": "工具超时（毫秒）",
    "approvalsEnabled": "工具审批",
    "approvalTimeoutMs": "审批超时（毫秒）",
    "permanentAllow": "永久允许的工具",
    "codeExecEnabled": "代码执行",
    "codeExecLanguages": "代码执行语言",
    "codeExecTimeoutMs": "代码执行超时（毫秒）",
    "codeExecMaxCalls": "代码执行工具调用上限",
    "maxOutputBytes": "最大输出字节数",
    "maxStderrBytes": "最大错误输出字节数",
    "maxSourceBytes": "最大源码字节数",
    "memoryEnabled": "启用记忆",
    "recentMessageLimit": "最近消息数量",
    "recallTokenBudget": "自动召回 Token 预算",
    "memorySearchLimit": "记忆搜索结果上限",
    "skillsEnabled": "启用 Skills",
    "skillReviewInterval": "每隔多少次工具调用检查 Skill",
    "mcpEnabled": "启用 MCP",
    "logMaxBytes": "日志文件最大字节数",
    "promptInstructions": "全局 Prompt 指令",
    ...
    "failureLimitHint": "同一工具连续失败达到该次数后，要求模型纠错或换方案，运行不会因此终止。",
    ...
    "statusDeleted": "已删除",
    "memoryTitle": "记忆",
    "memoryDescription": "查看和维护当前 Profile 的长期记忆。",
    "searchMemory": "搜索记忆标题或内容…",
    "noMemory": "没有匹配的记忆",
    "graphView": "关联图",
    "listView": "列表",
    "memoryNodes": "个节点",
    "memoryRelations": "条关系",
    "relationRevision": "版本演进",
    "relationSource": "同一来源",
    "relationEntity": "共享实体",
    ...
    "memoryGraphHint": "选择节点可查看完整内容和关联关系。",
    "editMemory": "编辑记忆",
    "deleteMemoryConfirm": "软删除这条记忆？",
    "titleLabel": "标题",
    "contentLabel": "内容",
    "tagsLabel": "标签",
    "tagsPlaceholder": "使用逗号分隔标签",
    "skillsTitle": "Ekko Skills",
    "skillsDescription": "创建和编辑 Ekko 可复用的工作指令。",
    "newSkill": "新建 Skill",
    "searchSkills": "搜索 Skills…",
    "noSkills": "没有匹配的 Skills",
    "selectSkill": "选择一个 Skill 或新建",
    "skillName": "Skill 名称",
    "skillContent": "SKILL.md",
    "skillFormatHint": "SKILL.md 必须包含 YAML frontmatter，且 name 与 Skill 名称一致，并包含 description 和精简的英文 metadata.keywords。",
    "skillTemplateDescription": "说明 Ekko 应在什么情况下使用这个 Skill。",
    "skillTemplateBody": "在这里编写 Skill 指令。",
    "deleteSkillConfirm": "归档这个 Skill？",
    "mcpTitle": "MCP 服务器",
    "mcpDescription": "管理 Ekko 运行时可用的本地及远程 MCP 服务。",
    "enabledServers": "已启用",
    "addServer": "添加服务",
    "editServer": "编辑服务",
    "managed": "Studio 管理",
    "custom": "自定义",
    "managedHint": "Studio 管理的服务由系统自动注入，也可以按 Profile 编辑、删除、启用或停用。",
    "noServers": "尚未配置 MCP 服务",
    "testServer": "测试",
    "testSuccess": "测试完成，发现 {count} 个工具",
    "testFailed": "连接测试失败",
    "deleteServerConfirm": "删除这个 MCP 服务？",
    "serverName": "服务名称",
    "serverNamePlaceholder": "例如：local-tools",
    "serverConfig": "服务配置（JSON）",
    "serverConfigHint": "本地 MCP 使用 command/args/env；远程 MCP 使用 type: streamable_http、url 和可选 headers。",
  ...
  agentManager: {
    ...
    path: '路径',
    userCli: 'Hermes CLI',
    userCliReadonly: '优先探测当前环境中的 Hermes CLI；CLI 由用户自行管理。',
    readOnly: '只读',
    noUserCli: '未探测到可用的 Hermes CLI',
    unknownVersion: '版本未知',
    inUse: '当前使用',
    builtIn: '内置',
    ...
    installOperation: '安装',
    deleteOperation: '删除',
    aiHelpDialogTitle: '{name} 操作失败',
    aiHelpDialogQuestion: '{name} {operation}失败，是否让 Ekko AI 帮你排查？',
    aiHelpDialogPositive: '询问 AI',
    ...
    legacyDataMigrationDescription: 'Ekko Studio 在旧版 Windows 数据目录中发现了数据。是否将其迁移到当前数据目录？',
    ...
    legacyDataMigrationSuccess: '已记录迁移选择，Ekko Studio 将重启并在服务启动前迁移数据。',
  ...
  runtimeVersions: {
    ...
    activePythonPath: '当前 Python 路径',
    activeAgentRoot: '当前 Agent Root',
    viewCliDetails: '查看',
    cliDetailsTitle: 'Hermes CLI 详情',
    activeDataDirectory: '当前数据目录',
    dataDirectoryEnvDescription: '当前目录可能被 HERMES_HOME 环境变量覆盖。如果目录不正确，请将 HERMES_HOME 设置为要使用的数据目录。',
    dataDirectoryEnvInstructions: '查看各平台环境变量设置',
    dataDirectoryEnvWindows: 'Windows PowerShell（当前用户）',
    dataDirectoryEnvMacos: 'macOS（桌面端 / 终端）',
    dataDirectoryEnvLinux: 'Linux（登录环境）',
    dataDirectoryEnvDocker: 'Docker Compose',
    dataDirectoryEnvRestart: '修改后请完全退出并重新打开 Ekko Studio。若 Windows 或 Linux 桌面端仍读取旧值，请注销并重新登录系统。',
    cliUpdateDescription: 'Hermes Runtime 0.19.1 及以上版本支持通过以下命令单独升级内置的 Hermes Agent；该命令不会升级 Ekko Studio 桌面端或 Web UI。执行前请先完全退出 Ekko Studio：',
    webUiTitle: 'Web UI',
    platform: '平台',
    ...
    runtimeDirectorySaved: 'Runtime 迁移已安排，重启 Ekko Studio 后执行。',
    runtimeMigrationPending: '待迁移到：{directory}',
    runtimeMigrationFailed: '存储目录迁移失败，已继续使用原目录',
    runtimeActivationFailed: '所选 Runtime 无法启动，Ekko Studio 已改用其他已安装版本',
    active: '已启用',
    installed: '已安装',
    download: '下载',
    ...
    activateSuccess: '版本选择已保存，重启 Ekko Studio 后生效。',
  ...
  connections: {
    ...
    app: {
      ...
      viewDownload: '下载',
      viewMessages: '消息推送',
      downloadTitle: '在手机上使用 Ekko Studio',
      downloadDescription: '我们正在内测阶段，需要注册码才有办法注册，我们会在社交媒体放出一部分注册码。',
      downloadPaidDescription: 'Ekko Studio App 已正式上线，目前处于付费阶段。如需体验，请前往定价与购买页面选择合适的方案，完成购买后即可开始使用。',
      downloadScan: '手机扫码下载',
      ...
      accessFailures: {
        ...
        cloudSubscriptionRequired: '当前账号未开通 Ekko Studio Cloud，无法使用云端连接',
  ...
  chat: {
    ...
    threadSegments: '共 {total} 段对话 · 当前第 {index} 段',
    ...
    pushEnabled: '是否推送',
    pushNotConfigured: '请先到 Ekko Studio「设备互联」中配置消息推送。',
    ...
    categoryCreatedAndMoved: '已创建分类“{name}”并移动会话',
    categoryCreatedMoveFailed: '已创建分类“{name}”，但未能移动会话。请重试以移动会话。',
    ...
    obsidianSaveFailed: '存入 Obsidian 失败',
    ...
    workspaceExtraDirs: '附加目录（agent 获得读写权限）',
    workspaceProject: '项目（多目录工作区）',
  ...
  settings: {
    ...
    display: {
      ...
      notifyOnApprovalHint: 'Ekko Studio 不在前台时，出现新的审批或澄清会发送系统通知；通知不显示敏感详情',
      ...
      approvalNotificationBody: '打开 Ekko Studio 查看并处理。',
    ...
    voice: {
      ...
      hermesConfigDescription: 'Hermes profile 中原有的原生提供商会继续保留；Web UI 管理的提供商对 Hermes 统一显示为 Ekko Studio。',
  ...
  changelog: {
    ...
    new_0_7_22_4: '修复 Hermes Bridge 使用 Studio 管理的 Claude OAuth 登录，并在本地通信路径过长时自动改用 TCP (#3033, #3048)',
    new_0_7_22_5: '补齐全局 Coding Agent 会话的 Studio MCP 工具，改进启动命令搜索路径及未安装可执行程序时的错误提示 (#3044, #3055)',
    new_0_7_22_6: '为 Studio 生成的 DSH 插件声明版本号，修复插件清单兼容性 (#3038)',
    new_0_7_22_7: '修复 API 地址配置迁移时，空 BASE_URL 错误读取下一行配置的问题 (#3046)',
    new_0_7_22_8: 'OpenRouter 请求中的应用归属统一显示为 Ekko Studio (#3056)',
    new_0_7_22_9: '修复 Grok 独立会话配置中的多行数组、长文本和表数组被截断或错误拆分的问题 (#3072)',
    new_0_7_22_10: '修复桌面运行时打包时浏览器执行权限和符号链接丢失的问题，兼容 Hermes 0.21.3；运行时发布不再覆盖桌面端的最新版本标记 (#3073)',
    new_0_7_21_1: '修复 Windows 下因选错启动脚本导致 DSH 插件配置和 Agent 预设页面无法打开的问题 (#3026)',
    new_0_7_21_2: 'DSH 配置运行时启动失败时显示具体阶段及系统错误码或退出码，不暴露插件日志和凭据 (#3026)',
    new_0_7_20_1: '新增 DeepSeek Harness（DSH）安装管理及单聊、群聊和工作流接入，支持选择 Agent 预设并在恢复会话时保留原有模式；同步中英文使用说明 (#3020, #3023)',
    new_0_7_20_2: '新增 DSH 原生 Web 插件配置、插件列表和 Agent 预设管理，支持复制预设、设置默认项及删除自定义预设；需在后端主机安装 DSH 和 Web 依赖 (#3020)',
    new_0_7_20_3: '群聊 Agent 新建和编辑改为右侧抽屉，表单可滚动、操作按钮固定，移动端使用全屏宽度 (#3020)',
    new_0_7_20_4: '所有 Coding Agent 页面中的共享技能改为只读，防止误编辑或删除；DSH 私有技能仍可编辑 (#3020)',
    new_0_7_20_5: 'Pi 安装和更新跟随 npm 最新版本，优先使用用户已启用的 MCP 适配器，避免重复注册工具和参数 (#2999, #2991)',
    new_0_7_20_6: '修复全局 Coding Agent 用量统计和模型归属，保留已完成响应的用量并改进缓存 Token 统计；历史记录不回填 (#3005)',
    new_0_7_20_7: '修复 Grok 失败轮次用量丢失、自定义服务商上下文范围，以及修改推理强度后运行配置未刷新的问题 (#3002)',
    new_0_7_20_8: 'Docker 中通过 Studio 安装的 Coding Agent 随 Studio 数据卷持久保存，重建或更新容器后不再丢失；旧镜像安装的 Agent 需重新安装一次 (#3006)',
    new_0_7_20_9: '隐藏 OpenCode 不支持的 /compact 命令，手动输入时给出说明且不打断当前任务 (#3010)',
    new_0_7_20_10: '修复 Hermes Agent 0.21 及后续版本的审批会话关联，兼容旧版运行时 (#3018)',
    new_0_7_20_11: '将 apikey 默认域名更新为 apikey.fan，启动时一次性迁移已有 Hermes 配置中的旧端点，保留路径、密钥和模型设置 (#3022)',
    new_0_7_20_12: '优化 Linux 启动器和窗口图标的圆角与透明留白，使显示尺寸更协调 (#3008)',
    new_0_7_19_1: '品牌升级为 Ekko Studio，统一应用图标、官网域名、CLI 命令与产品文档 (#2966, #2980, #2981, #2992, #2994)',
    new_0_7_19_2: '修复桌面端反复重启、更新中断与启动失败；升级后保留登录及浏览器数据，并迁移 Windows 开机启动设置 (#2931, #2979, #2988)',
    new_0_7_19_3: '新增原生 OpenCode Free 提供商，无需 API Key 即可建聊和供 Coding Agent 使用，可用模型在后台加载 (#2932)',
    new_0_7_19_4: '修复 Ekko 与 Studio 代理的 Coding Agent 请求 OpenCode 时的会话兼容性；Hermes 仍需升级运行时 (#2996)',
    new_0_7_19_5: 'Ekko 新增可持久化的任务计划与实时进度卡片；后台委派任务未结束时，侧栏继续显示工作状态 (#2952, #2961)',
    new_0_7_19_6: '聊天文件链接可直接定位并高亮引用行；工作区文件树支持无留白折叠，保留文件与目录状态 (#2947, #2965)',
    new_0_7_19_7: '桌面端可选择在内置或系统浏览器打开网页链接，内置浏览器面板立即展开，无需等待页面加载完成 (#2928, #2956)',
    new_0_7_19_8: '浏览器标注支持逐项删除和撤销，并修复关闭标签页或清理标注时的异常 (#2949, #2944)',
    new_0_7_19_9: '朗读跳过隐藏思考内容并保留负号、百分号等语义符号；豆包 TTS 支持 0.5–2 倍语速并同步当前配置 (#2945, #2950, #2978)',
    new_0_7_19_10: '恢复 Hermes 与 Ekko 技能列表默认显示的启用和禁用开关 (#2933)',
    new_0_7_19_11: '配合新版 App，新增聊天审批、澄清、任务结束、群聊 Agent 回复和工作流完成或失败的前台通知，并修复断连后的中继清理 (#2940, #2946)',
    new_0_7_19_12: '配合新版 App，会话分类支持独立分页加载，并正确返回置顶筛选结果与分类总数 (#2977, #2982)',
    new_0_7_19_13: '单聊中经用户确认可只读获取 iOS 步数、睡眠、心率等健康数据；需兼容版本 App，暂不支持 Android (#2972)',
    new_0_7_19_14: '新增 Studio 公告弹窗，仅展示最新公告并记住关闭状态，获取失败不影响使用 (#2989)',
    new_0_7_19_15: '修复 Codex 多行配置和 Hook 表的保留问题、重启配置过早超时，以及 Docker 可选补丁失败阻塞启动的问题 (#2951, #2960, #2969)',
    new_0_7_19_16: '加入 Agent 自动更新策略基础代码；尚未接入界面或后台更新，不会自动升级 Agent (#2923)',
    new_0_7_18_1: '新增 OpenCode Coding Agent，支持会话续接、流式工具调用、配置、记忆、Skills、工作流和群聊 (#2890)',
    new_0_7_18_2: '群聊现在支持通过云端中继连接远程 Agent，改善流式输出顺序、断线重连、摘要恢复与分享图片访问，并统一工具折叠和移动端布局 (#2927)',
    new_0_7_18_3: '新增手机单次定位、日历与提醒事项能力；敏感操作需用户确认，删除操作绑定已确认的具体条目，并完善授权超时与请求来源校验 (#2820, #2926)',
    new_0_7_18_4: '统一会话操作菜单，支持移动会话时新建分类，刷新后保留分类折叠状态，并新增 Ctrl/Cmd + , 打开设置的快捷键 (#2887, #2896, #2910, #2912)',
    new_0_7_18_5: '支持发送前预览已上传图片、预览代码样式的本地文件链接与工作区 Markdown，并优化消息引用、恢复回复箭头 (#2885, #2893, #2903, #2908)',
    new_0_7_18_6: 'Runtime 下载轮询现在仅对超级管理员按需运行；Windows 解压采用独立目录并在系统 tar 不可用时回退到 Node tar，同时修复分离式 Hermes Runtime 的 Bridge MCP 导入 (#2925)',
    new_0_7_18_7: 'Coding Agent 现在会隔离启动失败的 MCP Runtime 并清理已移除的服务；Studio 管理的 MCP 启动器明确使用 Node 模式，避免 Electron 误开桌面窗口 (#2888, #2920)',
    new_0_7_18_8: 'Ekko 现在可从重复工具失败中恢复，改善工具重试与失败处理 (#2891)',
    new_0_7_18_9: '修复空环境变量值跨行读取的问题，避免下一行配置被误当作模型地址或密钥 (#2884)',
    new_0_7_18_10: '应用下载页新增购买入口，并根据版本清单中的访问模式显示 (#2895)',
    ...
    new_0_7_11_4: '升级时会一次性以内置 Skills 替换整个 Skill 目录并原子写入完成标识，避免后续启动在 Windows 删除 Git 目录时崩溃（#2792）',
    new_0_7_11_5: '启动流程现可防御畸形 WebSocket 请求以及日志、Runtime、Ekko 或数据库初始化异常；Desktop 会快速显示界面，并在后端或渲染进程退出时恢复，不改变正常成功流程（#2792）',
    new_0_7_1_1: 'Ekko 记忆写入和遗忘现会直接更新存储，无需模型审批或后台审查队列，避免因审查超时重新带回旧记忆卡片（#2785）',
    new_0_7_1_2: '已彻底移除隐藏的滚动会话摘要链路，以及相关审查任务、配置、API、界面状态和数据库表（#2785）',
    new_0_7_1_3: '列出全部和清空全部记忆现可可靠执行；记忆工具失败会明确中止运行；数据库升级支持锁重试、原库备份、结构重建和兼容数据恢复（#2785）',
    new_0_7_0_1: '本版本涵盖 v0.6.47 之后合并的全部 21 个 PR，重点完成 Ekko 深度集成、统一 Agent 运行时管理、增强聊天状态保留，并提升 Studio 的兼容性与可靠性',
    new_0_7_0_2: 'Ekko 现已成为由 Studio 完整管理的运行时，支持 Profile Agent、对话、托管记忆、内置及外部技能、MCP 服务和独立设置页面（#2752、#2770）',
    new_0_7_0_3: 'Ekko 现可并行执行安全的工具调用、持久化带版本的全局 Agent 配置，将包括单文件 Navicat 友好型 ekko.db 在内的全部开发数据统一放在 packages/ekko-agent/.ekko，并且只自动安装自己的专属技能，不再复制 Hermes 技能（#2772、#2775、#2776）',
    new_0_7_0_4: '统一的“Agents”管理器现负责 Hermes Runtime 的发现、选择、启动、重启、供应商令牌和实时状态；进入页面时直接读取缓存状态，仅在点击刷新时执行耗时扫描（#2760、#2761）',
    new_0_7_0_5: '聊天现可在导航后保留思考耗时、按运行隔离重复使用的工具调用 ID，并在停止、清理或服务重启后续接全局 Coding Agent 上下文（#2723、#2730、#2735）',
    new_0_7_0_6: '附件传输现支持正确内容类型、分块上传下载、进度、取消、安全文件名和视频预览；Codex 请求也会移除历史内联图片，同时保留当前轮次的图片（#2732、#2773）',
    new_0_7_0_7: '从“最近”选择聊天时，会在导航和刷新后继续保留其真实分类的折叠状态；单聊和群聊也会正确显示 Profile、成员及 Agent 身份（#2742、#2754、#2755）',
    new_0_7_0_8: '全局 Coding Agent 聊天不再显示无法生效的推理强度选择器；微信绑定通知也补充了推送限制及恢复通知的方法（#2728、#2731）',
    new_0_7_0_9: '服务端职责现规范划分到 Studio、Hermes、Ekko 与 Coding Agent 模块；旧版 MCU 路由继续兼容，关闭 Studio 时也会可靠清理其创建的进程树（#2744、#2746、#2747）',
