...
describe('App connections scan modal', () => {
  it('switches between the connection list, mobile download hub, and message push', () => {
    ...
    expect(source).toContain('mobileRelease.value.channels.googlePlay.version')
    expect(source).toContain('mobileRelease.value.channels.apple.version')
    expect(source).toContain('APK {{ androidVersionLabel }}')
    expect(source).toContain('Google Play {{ googlePlayVersionLabel }}')
    expect(source).toContain('iOS {{ iosVersionLabel }}')
  ...
  it('offers LAN and cloud QR modes with manual refresh and an expired overlay', () => {
    ...
    expect(source).toContain('accessFailureTime')
    expect(source).toContain('new Intl.DateTimeFormat(locale.value')
    expect(source).not.toContain('new Date(accessFailure.occurredAt).toLocaleString()')
    ...
    expect(source).toContain('v-if="accessFailureRequiresPurchase"')
    expect(source).toContain(':href="APP_ACCESS_PURCHASE_URL"')
    expect(source).toContain('target="_blank"')
    expect(source).toContain('rel="noopener noreferrer"')
    expect(source).toContain("t('connections.app.purchaseAccess')")
    expect(source).toContain('visibleFailure.occurredAt > previousFailureAt && showScanModal.value')
